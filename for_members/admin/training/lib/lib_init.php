...
function edit_init(){
  global $pdo, $filter_date, $grades, $members, $participations;
  ...
  $sql = "SELECT member_id FROM training_participations WHERE date = '" . $filter_date ."'";
...
function view_init(){
  ...
  $sql_date = isset($_GET['filter_date']) ? "date >= '".$_GET['filter_date']."'" : 1;
