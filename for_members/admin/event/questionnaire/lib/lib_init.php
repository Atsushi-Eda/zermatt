...
function view_init(){
  ...
  $sql = "SELECT * FROM events WHERE questionnaire = 1 AND {$sql_event} ORDER BY date DESC";
...
function text_init(){
  ...
  if($event['meeting_place'] == ''){
    foreach($sql_grades as $key => $sql_grade){
      $sql = "SELECT m.nickname, p.after FROM event_participations AS p JOIN members AS m ON p.member_id = m.id WHERE p.event_id = {$_GET['event_id']} AND p.participation = 1 AND {$sql_grade} ORDER BY m.grade ASC, m.order ASC";
...
function edit_init(){
  ...
  if(empty($_POST)){
    ...
    header('Location: http://' . ROOT_DIR . '/for_members/admin/event/questionnaire/view.php?event_id=' . $_POST['event_id']);
    exit;
  }
}
