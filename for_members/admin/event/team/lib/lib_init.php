...
function index_init(){
  ...
  $sql = "SELECT * FROM events WHERE grade = ".MANAGER_GRADE." AND sophomore = 0 AND questionnaire = 1 ORDER BY date DESC";
  $events = $pdo->query($sql)->fetchAll(PDO::FETCH_ASSOC);
  if(isset($_GET['event_id'])){
    $sql = "SELECT short_name FROM events WHERE sophomore = 0 AND id = {$_GET['event_id']}";
...
function edit_init(){
  ...
  if(!isset($_POST['event_id'])){
    if(!isset($_GET['event_id'])){
      ...
      header('Location: http://' . ROOT_DIR . '/for_members/admin/event/team/');
      exit;
    }
    $sql = "SELECT name, date FROM events WHERE sophomore = 0 AND id = {$_GET['event_id']}";
    $event = $pdo->query($sql)->fetch(PDO::FETCH_ASSOC);
    $sql = "SELECT team FROM event_teams WHERE event_id = {$_GET['event_id']} ORDER BY team DESC";
    $team_tmp = $pdo->query($sql)->fetch(PDO::FETCH_ASSOC);
    $teams = $team_tmp['team'];
    ...
    foreach($pdo->query($sql)->fetchAll(PDO::FETCH_ASSOC) as $participant){
      $sql = "SELECT e.id, e.short_name, t.team FROM event_teams AS t JOIN events AS e ON t.event_id = e.id WHERE e.sophomore = 0 AND t.member_id = {$participant['member_id']} AND e.date < CAST('{$event['date']}' AS DATE) ORDER BY e.date DESC LIMIT 2";
    ...
    header('Location: http://' . ROOT_DIR . '/for_members/admin/event/team/?event_id=' . $_POST['event_id']);
    exit;
  }
}
